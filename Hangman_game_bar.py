...
# Function to visualize correct and incorrect guesses with a custom bar chart
def visualize_guesses(guess_sequence):
    ...
    for guess, is_correct in guess_sequence:
    ...
    ax.bar(range(len(x_labels)), [1] * len(x_labels), color=colors)
    ...
    plt.ylim(0, 1.5)  # Keep all bars the same height for uniformity
